...
class FastKNNClassifier(Classifier):
    ...
    def fit(self,X,T):
        ...
        self.kdtree = scipy.spatial.KDTree(self.X)                         # !!REPLACE!! do an indexing of the feature vectors by constructing a kd-tree
    ...
    def predict(self,x,K=None):
        ...
        dist,idxKNN = self.kdtree.query(x,K)                            # !!REPLACE!! get indexes of K nearest neighbors of x from kd-tree
...
class KernelMLPClassifier(Classifier):
    ...
    def fit(self,X,T):
        ...
        lambda1=0
        self.Wy= np.dot(T.T, np.linalg.inv(np.tanh(self.K)+ lambda1* np.eye(len(X))))    # !!REPLACE!! weight matrix from hidden to output layer 
    ...
    def predict(self,x):
        ...
        :returns None: dummy 
        """
        z=np.tanh(np.dot(self.Wz,x))                        # !!REPLACE!! firing rates of hidden layer z
